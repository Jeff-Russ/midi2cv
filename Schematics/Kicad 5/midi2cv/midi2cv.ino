...
#define TRIG  3
#define CLOCK 4
#define DAC1  8 // chip select, see dacpin variable
#define DAC2  9 // chip select, see dacpin variable
// The following are standard for SPI on arduino uno/nano
// and therefore don't need to be specifed:
// D11 is data "MOSI" (to both DAC chips) and
// D13 is clock "SCK" (to both DAC chips)
  ...
  if (MIDI.read()) {                    
    ...
    switch (type) {
      ...
      case midi::NoteOff:
        ...
        if (velocity == 0) notes[noteMsg] = false;
        else {
          notes[noteMsg] = true;
        ...
        if (S1 && S2) commandTopNote(); // Highest note priority
        else if (!S1 && S2) commandBottomNote(); // Lowest note priority
      ...
      case midi::Clock:
        ...
        clock_count++;
        if (clock_count == 24) clock_count = 0; // MIDI timing clock sends 24 pulses per quarter note.  Sent pulse only once every 24 pulses
        break;
...
void commandTopNote()
{
  ...
  if (noteActive) commandNote(topNote);
  else digitalWrite(GATE,LOW);// All notes are off, turn off gate
}
...
void commandBottomNote()
{
  ...
  if (noteActive) commandNote(bottomNote);
  else digitalWrite(GATE,LOW); // All notes are off, turn off gate
}
